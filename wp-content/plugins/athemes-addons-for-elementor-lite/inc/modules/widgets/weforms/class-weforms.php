...
class weForms extends Widget_Base {
	...
	public function get_style_depends() {
		return [ $this->get_name() . '-styles', 'weforms-css' ];
	}
	...
	public function get_keywords() {
		return [ 'contact', 'form', 'contact form', 'weforms', 'athemes', 'addons', 'athemes addons' ];
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'selectors' => [
					'{{WRAPPER}} .wpuf-form-add.wpuf-style ul.wpuf-form .wpuf-submit input[type="submit"]' => 'fill: {{VALUE}}; color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .wpuf-form-add.wpuf-style ul.wpuf-form .wpuf-submit input[type="submit"]:hover svg, {{WRAPPER}} .wpuf-form-add.wpuf-style ul.wpuf-form .wpuf-submit input[type="submit"]:focus svg' => 'fill: {{VALUE}};',
				],
			]
		);
	...
	public function forms() {
		...
		if ( empty( $forms_list ) ) {
			...
			$forms_list = wp_list_pluck( $forms_list, 'post_title', 'ID' );
		}
	...
	protected function render() {
		...
		<div class="athemes-addons-contact-form athemes-ele-weforms">
			<div class="athemes-addons-form-inner">
				...
				if ( !empty( $settings['contact_forms'] ) ) {
					echo do_shortcode( '[weforms id="' . $settings['contact_forms'] . '"]' );
				}
				?>
			</div>
		</div>
...
Plugin::instance()->widgets_manager->register( new weForms() );